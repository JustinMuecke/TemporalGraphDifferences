    ...
    private HashMap<Integer, Integer> getFlowNodes(Graph<Integer, Edge> component, ExtGraph graph1, ExtGraph graph2){
        ...
        for(Integer vertex : component.vertexSet()) {
            ...
            if (graph2.getSecondaryIndex().containsKey(vertex)) {
                if (graph1.getSecondaryIndex().get(vertex) > graph2.getSecondaryIndex().get(vertex)) {
                    sourceNodes.put(vertex, Math.abs(graph1.getSecondaryIndex().get(vertex) - graph2.getSecondaryIndex().get(vertex)));
                }
                continue;
            }
            sourceNodes.put(vertex, graph1.getSecondaryIndex().get(vertex));
        }
        return sourceNodes;
    }
