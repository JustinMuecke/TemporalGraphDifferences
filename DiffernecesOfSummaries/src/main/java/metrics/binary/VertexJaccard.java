...
public class VertexJaccard implements BinaryMetric {
    ...
    public Result compute(ExtGraph graph1, ExtGraph graph2) {
        ...
        try{
            ...
            float inUnion = (long) graph1.getSecondaryIndex().keySet().size() + graph2.getSecondaryIndex().keySet().stream().filter(key -> !graph1.getSecondaryIndex().containsKey(key)).count();
