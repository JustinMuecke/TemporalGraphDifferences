...
class ConfigPipeline(config: MyConfig, skipSnapshots: Int = 0, endEarly: Int = Int.MaxValue) {
  ...
  def start(): ChangeTracker = {
    ...
    while (iterator.hasNext && iteration <= endEarly) {
      ...
      val secondaryIndexFile = "secondaryIndex-"+ iteration 

      var lastIteration : Int = iteration-1
      if (skipSnapshots <= iteration) {
        ...
        if (!onlyBatch) {
          ...
          //unlike before, each iteration needs to create its own databse 
          OrientConnector.create(database+"-"+iteration, config.getBoolean(config.VARS.igsi_clearRepo))
          //else{
            //OrientConnector.createCopy(database+"-"+iteration, database+"-"+lastIteration,  config.getBoolean(config.VARS.igsi_clearRepo),trackPrimaryChanges, trackUpdateTimes, maxCoresInt)
          //  OrientConnector.getInstance(database+"-"+lastIteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).open(maxCoresInt)
          //}
          ...
          //if (iteration == 0 || iteration == skipSnapshots)
            OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).
          ...
          //else{
          //  OrientConnector.getInstance(database+"-"+lastIteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).
          //    setSecondaryIndex(SecondaryIndex.
          ...
          val igsi = new IGSI(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes)
          ...
          val graphDatabase: OrientConnector = OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt)
          ...
          while (deleteIterator.hasNext) {
            ...
            val imprintsResult = OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).
          ...
          val deleteResult = OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).bulkDeleteSchemaElements(schemaIDsToBeDeleted);
          logger.info("Deletion Complete")

          if (!deltaGraphUpdates)
            deleteResult.mergeAll(OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).removeOldImprintsAndElements(startTime))
          ...
          if (OrientConnector.FAKE_MODE) {
            ...
            val secondaryIndex = OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).
              getSecondaryIndex
            ...
            val indexSize = OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).countSchemaElementsAndLinks()
          ...
          if (trackTertiaryChanges) {
            val countStats = OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).completenessAnalysisExport()
            ...
            val tmpStats = OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).getIndexInformation()
            if (schemaStats == null)
          ...
          OrientConnector.getInstance(database+"-"+iteration, trackPrimaryChanges, trackUpdateTimes, maxCoresInt).close()
        ...
        if (alsoBatch) {
          ...
          OrientConnector.create(database+"-"+iteration + "_batch", true)
          if (trackPrimaryChanges || trackSecondaryChanges)
            Result.getInstance().resetScores()
          ...
          // batch writing
          val igsiBatch = new IGSI(database +"-"+iteration+ "_batch", trackPrimaryChanges, trackUpdateTimes)
          ...
          OrientConnector.getInstance(database+"-"+iteration + "_batch", trackPrimaryChanges, trackUpdateTimes, maxCoresInt).close()
          OrientConnector.removeInstance(database+"-"+iteration + "_batch")
